...
if __name__ == "__main__":
    import time
    htoken = HmacToken("SAMPLE SALT", TokenPeriod.minute)
    login = "login"
    times, token = htoken.getToken(login)
    print("Token {} is {}".format(token, "valid" if htoken.checkToken(login, times, token) else "invalid"))
    time.sleep(61)
    print("Token {} is {}".format(token, "valid" if htoken.checkToken(login, times, token) else "invalid"))
